...
fn check_basic_calc_operator(env: &Env, left: &Expression, right: &Expression) -> Result<TypeRef> {
    ...
    if left_type.type_name() != "int" {
        errors.push(format!("type error. left is should be `int`, but {}", left_type.type_name()));
    }
    let right_type = check_expression(env,right)?;
    if right_type.type_name() != "int" {
        errors.push(format!("type error. right should be `int`, but {}", right_type.type_name()));
    }
    if !errors.is_empty() {
...
";
        ...
        if let Some(Error{ errors }) = result.err() {
            ...
            assert_eq!(true, errors[1].starts_with("type error. right should be `int`, but"), "actual message: `{}`", errors[1]);
            assert_eq!(true, errors[2].starts_with("type error. initialize variable type is"), "actual message: `{}`", errors[2]);
            assert_eq!(true, errors[3].starts_with("type error. right should be `int`, but"), "actual message: `{}`", errors[3]);
